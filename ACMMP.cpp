                  ...
                  const int line)
{
    ...
    {
        std::cerr << StringPrintf("%s in %s at line %i",
                                  cudaGetErrorString(error), file.c_str(), line)
                  << std::endl;
        exit(EXIT_FAILURE);
    }
}
...
Camera ReadCamera(const std::string &cam_path)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        file >> camera.R[3 * i + 0] >> camera.R[3 * i + 1] >>
            camera.R[3 * i + 2] >> camera.t[i];
    }
    ...
    for (int i = 0; i < 3; ++i)
    {
        file >> camera.K[3 * i + 0] >> camera.K[3 * i + 1] >>
            camera.K[3 * i + 2];
    }
...
float3 Get3DPointonWorld(const int x, const int y, const float depth,
                         const Camera camera)
{
    float3 pointX;
    float3 tmpX;
...
float3 Get3DPointonRefCam(const int x, const int y, const float depth,
                          const Camera camera)
{
    float3 pointX;
    // Reprojection
...
void ProjectonCamera(const float3 PointX, const Camera camera, float2 &point,
                     float &depth)
{
    float3 tmp;
...
int writeNormalDmb(const std::string file_path,
                   const cv::Mat_<cv::Vec3f> normal)
{
    FILE *outimage;
    outimage = fopen(file_path.c_str(), "wb");
    if (!outimage)
...
#pragma omp parallel for
    for (size_t i = 0; i < pc.size(); i++)
    {
        ...
        if (!(X.x < FLT_MAX && X.x > -FLT_MAX) ||
            !(X.y < FLT_MAX && X.y > -FLT_MAX) ||
...
static float GetDisparity(const Camera &camera, const int2 &p,
                          const float &depth)
{
    float point3D[3];
    ...
    return std::sqrt(point3D[0] * point3D[0] + point3D[1] * point3D[1] +
                     point3D[2] * point3D[2]);
}
...
void ACMMP::SetHierarchyParams() { params.hierarchy = true; }

void ACMMP::SetPlanarPriorParams() { params.planar_prior = true; }

void ACMMP::InuputInitialization(const std::string &dense_folder,
                                 ...
                                 const int idx)
{
    ...
    for (size_t i = 0; i < images.size(); ++i)
    {
        ...
        {
            max_image_size =
                problems[problem.src_image_ids[i - 1]].cur_image_size;
        }

        if (images[i].cols <= max_image_size &&
        ...
        const float factor_x =
            static_cast<float>(max_image_size) / images[i].cols;
        const float factor_y =
            static_cast<float>(max_image_size) / images[i].rows;
        ...
        cv::resize(images[i], scaled_image_float, cv::Size(new_cols, new_rows),
                   0, 0, cv::INTER_LINEAR);
...
void ACMMP::CudaSpaceInitialization(const std::string &dense_folder,
                                    const Problem &problem)
{
    ...
    for (int i = 0; i < num_images; ++i)
    {
        ...
        cudaMemcpy2DToArray(cuArray[i], 0, 0, images[i].ptr<float>(),
                            images[i].step[0], cols * sizeof(float), rows,
        ...
        cudaCreateTextureObject(&(texture_objects_host.images[i]), &resDesc,
                                &texDesc, NULL);
    }
    cudaMalloc((void **)&texture_objects_cuda, sizeof(cudaTextureObjects));
    cudaMemcpy(texture_objects_cuda, &texture_objects_host,
               sizeof(cudaTextureObjects), cudaMemcpyHostToDevice);

    cudaMalloc((void **)&cameras_cuda, sizeof(Camera) * (num_images));
    cudaMemcpy(cameras_cuda, &cameras[0], sizeof(Camera) * (num_images),
               cudaMemcpyHostToDevice);

    plane_hypotheses_host = new float4[cameras[0].height * cameras[0].width];
    cudaMalloc((void **)&plane_hypotheses_cuda,
               sizeof(float4) * (cameras[0].height * cameras[0].width));

    costs_host = new float[cameras[0].height * cameras[0].width];
    cudaMalloc((void **)&costs_cuda,
               sizeof(float) * (cameras[0].height * cameras[0].width));
    cudaMalloc((void **)&pre_costs_cuda,
               sizeof(float) * (cameras[0].height * cameras[0].width));

    cudaMalloc((void **)&rand_states_cuda,
               sizeof(curandState) * (cameras[0].height * cameras[0].width));
    cudaMalloc((void **)&selected_views_cuda,
               sizeof(unsigned int) * (cameras[0].height * cameras[0].width));

    cudaMalloc((void **)&depths_cuda,
               sizeof(float) * (cameras[0].height * cameras[0].width));
    ...
    if (params.geom_consistency)
    {
        for (int i = 0; i < num_images; ++i)
        {
            ...
            cudaMemcpy2DToArray(cuDepthArray[i], 0, 0, depths[i].ptr<float>(),
                                depths[i].step[0], cols * sizeof(float), rows,
                                cudaMemcpyHostToDevice);
            ...
            cudaCreateTextureObject(&(texture_depths_host.images[i]), &resDesc,
                                    &texDesc, NULL);
        }
        cudaMalloc((void **)&texture_depths_cuda, sizeof(cudaTextureObjects));
        cudaMemcpy(texture_depths_cuda, &texture_depths_host,
                   sizeof(cudaTextureObjects), cudaMemcpyHostToDevice);
        ...
        cudaMemcpy(plane_hypotheses_cuda, plane_hypotheses_host,
                   sizeof(float4) * width * height, cudaMemcpyHostToDevice);
        cudaMemcpy(costs_cuda, costs_host, sizeof(float) * width * height,
                   cudaMemcpyHostToDevice);
    }
    ...
    if (params.hierarchy)
    {
        ...
        cudaMalloc((void **)&scaled_plane_hypotheses_cuda,
        ...
        cudaMemcpy(scaled_plane_hypotheses_cuda, scaled_plane_hypotheses_host,
                   sizeof(float4) * height * width, cudaMemcpyHostToDevice);
        cudaMemcpy(plane_hypotheses_cuda, plane_hypotheses_host,
                   sizeof(float4) * cameras[0].width * cameras[0].height,
                   cudaMemcpyHostToDevice);
    }
}

void ACMMP::CudaPlanarPriorInitialization(
    const std::vector<float4> &PlaneParams, const cv::Mat_<float> &masks)
{
    ...
    cudaMemcpy(prior_planes_cuda, prior_planes_host,
               sizeof(float4) * (cameras[0].height * cameras[0].width),
               cudaMemcpyHostToDevice);
    cudaMemcpy(plane_masks_cuda, plane_masks_host,
               sizeof(unsigned int) * (cameras[0].height * cameras[0].width),
               cudaMemcpyHostToDevice);
}

int ACMMP::GetReferenceImageWidth() { return cameras[0].width; }

int ACMMP::GetReferenceImageHeight() { return cameras[0].height; }

cv::Mat ACMMP::GetReferenceImage() { return images[0]; }
...
float ACMMP::GetCost(const int index) { return costs_host[index]; }

float ACMMP::GetMinDepth() { return params.depth_min; }

float ACMMP::GetMaxDepth() { return params.depth_max; }
...
std::vector<Triangle> ACMMP::DelaunayTriangulation(
...
float4 ACMMP::GetPriorPlaneParams(const Triangle triangle,
                                  const cv::Mat_<float> depths)
{
    ...
    float3 ptX1 =
        Get3DPointonRefCam(triangle.pt1.x, triangle.pt1.y,
                           depths(triangle.pt1.y, triangle.pt1.x), cameras[0]);
    float3 ptX2 =
        Get3DPointonRefCam(triangle.pt2.x, triangle.pt2.y,
                           depths(triangle.pt2.y, triangle.pt2.x), cameras[0]);
    float3 ptX3 =
        Get3DPointonRefCam(triangle.pt3.x, triangle.pt3.y,
                           depths(triangle.pt3.y, triangle.pt3.x), cameras[0]);
    ...
    A.at<float>(2, 3) = 1.0;
    cv::SVD::solveZ(A, B);
    float4 n4 = make_float4(B.at<float>(0, 0), B.at<float>(1, 0),
                            B.at<float>(2, 0), B.at<float>(3, 0));
...
float ACMMP::GetDepthFromPlaneParam(const float4 plane_hypothesis, const int x,
                                    const int y)
{
    return -plane_hypothesis.w * cameras[0].K[0] /
           ((x - cameras[0].K[2]) * plane_hypothesis.x +
            (cameras[0].K[0] / cameras[0].K[4]) * (y - cameras[0].K[5]) *
                plane_hypothesis.y +
            cameras[0].K[0] * plane_hypothesis.z);
}

void JBUAddImageToTextureFloatGray(std::vector<cv::Mat_<float>> &imgs,
                                   cudaTextureObject_t texs[],
                                   cudaArray *cuArray[], const int &numSelViews)
{
    for (int i = 0; i < numSelViews; i++)
    {
        ...
        cudaMallocArray(&cuArray[i], &channelDesc, cols, rows);
        cudaMemcpy2DToArray(cuArray[i], 0, 0, imgs[index].ptr<float>(),
                            imgs[index].step[0], cols * sizeof(float), rows,
                            cudaMemcpyHostToDevice);
...
void RunJBU(const cv::Mat_<float> &scaled_image_float,
            const cv::Mat_<float> &src_depthmap,
            const std::string &dense_folder, const Problem &problem)
{
    uint32_t rows = scaled_image_float.rows;
    uint32_t cols = scaled_image_float.cols;
    int Imagescale = std::max(scaled_image_float.rows / src_depthmap.rows,
                              scaled_image_float.cols / src_depthmap.cols);
