...
void RunFusion(std::string &dense_folder, const std::vector<Problem> &problems,
               bool geom_consistency, double scale)
{
    size_t num_images = problems.size();
    std::string image_folder = dense_folder + std::string("/images");
    ...
    for (size_t i = 0; i < num_images; ++i)
    {
        ...
        cv::Mat_<cv::Vec3b> image =
            cv::imread(image_path.str(), cv::IMREAD_COLOR);
        ...
        // rescale depth and normal to half size
        cv::Mat_<float> scaled_depth;
        cv::Mat_<cv::Vec3f> scaled_normal;
        std::cout << "Rescaling image " << std::setw(8) << std::setfill('0')
                  << i << "..." << std::endl;
        cv::resize(depth, scaled_depth,
                   cv::Size(depth.cols * scale, depth.rows * scale), 0, 0,
                   cv::INTER_NEAREST);
        cv::resize(normal, scaled_normal,
                   cv::Size(normal.cols * scale, normal.rows * scale), 0, 0,
                   cv::INTER_NEAREST);
        std::cout << "Rescaling image " << std::setw(8) << std::setfill('0')
                  << i << " done!" << std::endl;

        cv::Mat_<cv::Vec3b> scaled_image;
        RescaleImageAndCamera(image, scaled_image, scaled_depth, camera);
        images.push_back(scaled_image);
        cameras.push_back(camera);
        depths.push_back(scaled_depth);
        normals.push_back(scaled_normal);
        cv::Mat mask =
            cv::Mat::zeros(scaled_depth.rows, scaled_depth.cols, CV_8UC1);
        ...
        for (int r = 0; r < rows; ++r)
        {
            for (int c = 0; c < cols; ++c)
            {
                ...
                for (int j = 0; j < num_ngb; ++j)
                {
                    ...
                    if (src_c >= 0 && src_c < src_cols && src_r >= 0 &&
                        src_r < src_rows)
                    {
                        ...
                        float src_depth =
                            depths[src_id].at<float>(src_r, src_c);
                        cv::Vec3f src_normal =
                            normals[src_id].at<cv::Vec3f>(src_r, src_c);
                        if (src_depth <= 0.0) continue;

                        float3 tmp_X = Get3DPointonWorld(
                            src_c, src_r, src_depth, cameras[src_id]);
                        float2 tmp_pt;
                        ProjectonCamera(tmp_X, cameras[i], tmp_pt, proj_depth);
                        float reproj_error =
                            sqrt(pow(c - tmp_pt.x, 2) + pow(r - tmp_pt.y, 2));
                        float relative_depth_diff =
                            fabs(proj_depth - ref_depth) / ref_depth;
                        float angle = GetAngle(ref_normal, src_normal);

                        if (reproj_error < 2.0f &&
                            relative_depth_diff < 0.01f && angle < 0.174533f)
                        {
                            /* consistent_Point.x += tmp_X.x;
                             ...
                             consistent_Color[0] +=
                             images[src_id].at<cv::Vec3b>(src_r, src_c)[0];
                             consistent_Color[1] +=
                             images[src_id].at<cv::Vec3b>(src_r, src_c)[1];
                             consistent_Color[2] +=
                             images[src_id].at<cv::Vec3b>(src_r, src_c)[2];*/

                            used_list[j].x = src_c;
                            used_list[j].y = src_r;

                            float tmp_index = reproj_error +
                ...
                if (num_consistent >= 2 &&
                    (dynamic_consistency > 0.3 * num_consistent))
                {
                    ...
                    point3D.normal =
                        make_float3(consistent_normal[0], consistent_normal[1],
                                    consistent_normal[2]);
                    point3D.color =
                        make_float3(consistent_Color[0], consistent_Color[1],
                                    consistent_Color[2]);
                    ...
                    for (int j = 0; j < num_ngb; ++j)
                    {
                        ...
                        masks[image_id_2_index[problems[i].src_image_ids[j]]]
...
int main(int argc, char **argv)
{
    if (argc < 2)
    {
        std::cout << "USAGE: ACMMP dense_folder fusion_scale(optional)"
                  << std::endl;
        return -1;
    }
    ...
    RunFusion(dense_folder, problems, geom_consistency, scale);
